...
// Fetch only profile picture URL for a user
export const GetUserProfilePic = async (userId) => {
  try {

    const profileIdResponse = await apiClient.get(`/api/profiles/user/${userId}`);
    const profileIdData = profileIdResponse.data;
    ...
    console.warn(`Could not fetch profile picture for userId ${userId}`);
    return null;
  }
}

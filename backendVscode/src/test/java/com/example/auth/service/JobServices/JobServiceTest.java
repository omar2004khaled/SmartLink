...
class JobServiceTest {
        ...
        void getCurrentJobs_ValidCompany() {
                ...
                when(jobRepository.findByCompanyAndDeadlineAfter(
                                eq(testCompany),
                                any(Instant.class),
                                any(Pageable.class))).thenReturn(jobPage);
        ...
        @Test
        void deleteJob_WithNoApplications_ShouldDeleteSuccessfully() {
                when(jobRepository.findById(1L)).thenReturn(Optional.of(testJob));
                when(jobApplicationRepository.getApplicationByJobId(1L)).thenReturn(Optional.empty());
                doNothing().when(jobRepository).delete(testJob);

                jobService.deleteJob(1L);

                verify(jobRepository).findById(1L);
                verify(jobApplicationRepository).getApplicationByJobId(1L);
                verify(jobApplicationRepository, never()).deleteAll(any());
                verify(jobRepository).delete(testJob);
        }

        @Test
        void deleteJob_WithApplications_ShouldDeleteApplicationsFirst() {
                ...
                when(jobRepository.findById(1L)).thenReturn(Optional.of(testJob));
                when(jobApplicationRepository.getApplicationByJobId(1L)).thenReturn(Optional.of(applications));
                doNothing().when(jobApplicationRepository).deleteAll(applications);
                doNothing().when(jobRepository).delete(testJob);
                ...
                verify(jobRepository).findById(1L);
                verify(jobApplicationRepository).getApplicationByJobId(1L);
                verify(jobApplicationRepository).deleteAll(applications);
                verify(jobRepository).delete(testJob);
        }

        @Test
        void deleteJob_WithEmptyApplicationsList_ShouldNotDeleteApplications() {
                when(jobRepository.findById(1L)).thenReturn(Optional.of(testJob));
                when(jobApplicationRepository.getApplicationByJobId(1L))
        ...
        @Test
        void deleteJob_JobNotFound_ShouldThrowException() {
                ...
                verify(jobApplicationRepository, never()).getApplicationByJobId(any());
